...
export default function CashAtHomeScreen({ user, onNavigate }) {
  ...
  const getTranslatedStatus = (statusStr) => {
    ...
    if (s === 'TALLIED') return `✓ ${t('tallied') || 'TALLIED'}`;
    if (s === 'SHORT') return `⚠ ${t('short') || 'SHORT'}`;
    if (s === 'EXTRA') return `⚠ ${t('extra') || 'EXTRA'}`;
    return s;
  };
    ...
    <PageContainer>
      ...
      <div className="stitch-card" style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        <div style={{
          ...
          <span style={{ fontSize: '14px', fontWeight: '800', color: 'var(--navy-primary)' }}>
            = {t('expectedClosingCashAtHome')}
          </span>
          <span style={{ fontSize: '20px', fontWeight: '800', color: 'var(--navy-primary)' }}>
